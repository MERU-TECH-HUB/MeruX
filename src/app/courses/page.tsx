        ...
        <div className="space-y-8">
            ...
            {showCreateModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="bg-card border border-border/70 rounded-lg p-6 w-full max-w-lg shadow-2xl space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-5 mb-6 border-b border-border/70 pb-6">
                ...
                {(userRole === 'admin' || userRole === 'instructor' || user?.user_metadata?.role === 'admin') && (
                    <button onClick={() => setShowCreateModal(true)} className="inline-flex items-center gap-2 px-4 py-2.5 bg-hub-indigo text-white rounded-lg font-bold hover:bg-hub-indigo/90 transition-all text-sm shadow-sm active:scale-95">
            ...
            <div className="flex flex-col md:flex-row items-center gap-3 bg-card p-3 rounded-lg border border-border/70">
                <div className="relative flex-1 w-full">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                    <input
                        type="text"
                        placeholder="Search courses or topics..."
                        className="w-full bg-background border border-border/70 rounded-lg pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-hub-indigo/40 transition-all"
                    />
                </div>
                <div className="flex items-center gap-2">
                    <button className="inline-flex items-center gap-2 px-4 py-2.5 bg-background border border-border/70 rounded-lg hover:bg-accent transition-all text-sm font-medium">
...
function CourseListItem({ course, isEnrolled, onEnroll, isEnrolling, userRole, userId, onDelete, isDeleting }: any) {
    ...
    return (
        <div className="premium-card group flex flex-col h-full hover:border-hub-indigo/30">
