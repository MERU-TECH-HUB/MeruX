...
export function Sidebar({ className, onClose }: { className?: string; onClose?: () => void }) {
    ...
    return (
        <div className={cn("w-64 border-r border-border bg-card flex flex-col h-screen sticky top-0 z-50 shrink-0", className)}>
            <div className="p-4 flex flex-col gap-4 relative border-b border-border/70">
                {onClose && (
                ...
                <button
                    onClick={toggleTheme}
                    disabled={!mounted}
                    className="flex items-center gap-3 px-3 py-2 rounded-lg text-muted-foreground hover:bg-accent hover:text-foreground transition-all text-sm font-medium border border-border/70 bg-background"
                >
                    {isDark ? (
            ...
            <nav className="flex-1 px-3 overflow-y-auto custom-scrollbar py-4">
                {roleLoading ? (
                    ...
                    <div className="space-y-1">
                        {filteredSections.map((section, si) => (
                            <div key={si} className={cn(si > 0 && "pt-3")}>
                                {section.label && (
                                    <p className="px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.14em] text-muted-foreground/70">
                                        {section.label}
                                    </p>
                                )}
                                {section.items.map((item) => {
                                    ...
                                    return (
                                        <Link
                                            ...
                                            onClick={onClose}
                                            className={cn(
                                                "flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors duration-200 group text-sm font-medium",
                                                isActive
                                                    ? "bg-hub-indigo/10 text-hub-indigo border border-hub-indigo/20"
                                        ...
                                        >
                                            ...
                                            {item.badge && (
                                                <span className="ml-auto bg-hub-rose text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">
                                                    {item.badge}
                                                </span>
                                            )}
                                        </Link>
            ...
            <div className="p-4 border-t border-border space-y-1 bg-background/60">
                <button
                    onClick={handleLogout}
                    disabled={loggingOut}
                    className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-rose-500 hover:bg-rose-500/10 transition-all text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed group"
                >
                    {loggingOut ? (
